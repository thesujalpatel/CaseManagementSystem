...
exports.createuser = (req, res) => {
    ...
    userDB.findOne({ name: req.body.name })
        .then(existingUser => {
            if (existingUser) {
                ...
                const userinfo = new userDB({
                    ...
                    role: req.body.role,
                    phone: [req.body.phone1, req.body.phone2 == "" ? null : req.body.phone2],
                    createDate: Date.now(),
                    updateDate: null
                });
                userinfo.save()
        ...
        .catch(err => {
            res.status(500).send({ message: err.message || "Error occurred while checking user existence" });
        });
}
